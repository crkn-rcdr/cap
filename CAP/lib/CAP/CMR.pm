...
our $DEBUG  = 1;  # Output extended debugging information to stderr
...
sub cmr_media
{
    ...
    # Prevent certain types from being misinterpreted by catching them
    # here before we do any further processing
    s/still image/image/ig;
    s/moving image/video/ig;
    s/sound recording/audio/ig;
    s/text\/plain/plaintext/ig;
    s/application\///ig; # e.g. Application/PDF; remove for clarity
    ...
    { $codes{audio} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

    # Data
    foreach my $tag (
        'atlasti', 'data', 'dataset', 'excel', 'xml'
    )
    { $codes{data} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

    # Image
    foreach my $tag (
        'atlas', 'business card', 'drawing', 'graphic material',
        'holograph', 'ill', 'illustration', 'image', 'jpe?g', 'maps?', 'photo', 'photographs?',
        'photographic (negatives|positives)', 'photojournal',
        'postcard', 'poster', 'ppt', 'print', 'rotogravure',
        'scrapbooks?', 'slides?', 'tiff?'
    )
    { $codes{image} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

    # Plaintext
    foreach my $tag (
        'plaintext'
    )
    { $codes{plaintext} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

   # Text
    foreach my $tag (
        'announcement', 'article', 'atlas', 'binding', 'book', 'booklet', 'broadside',
        'correspondence', 'document', 'finding aid', 'journal', 'letter',
        'magazine', 'manuscript', 'minutes', 'newsletters?', 'newspaper', 'newsprint',
        'notebooks?', 'pamphlet', 'pdf', 'periodical', 'poems?', 'report', 'rtf', 'scrapbooks?',
        'short story',
        'texts?', 'textual \w+', 
        'thesis', 'theses',
        'typescript', 'yearbooks?'
    )
    { $codes{text} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

    # Video
    foreach my $tag (
        'moving_image', 'video', 'videorecording', 'wmv'
    )
    { $codes{video} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g; }

    s/^ +//; s/ +$//; s/\s+/ /g;
    warn("[CMR.pm DEBUG] Leftover <media> fragments: $_\n") if ($DEBUG && $_);
...
sub cmr_lang
{
    my $_ = join(' ', @_);
    my %codes  = ();
    $_ =~ s/mi'kmaq/mic/ig; # Avoid mis-interpretig Mi'kmaq as mi + kmaq.
    $_ =~ s/\W+/ /g;
    $_ = lc($_);
    ...
    { $codes{$1} = 1 if /\b($tag)\b/; $_ =~ s/\b$tag\b//g; }
    ...
    foreach my $tag (keys(%iso693)) {
        $codes{$iso693{$tag}} = 1 if /\b$tag\b/; $_ =~ s/\b$tag\b//g;
    }
