...
sub index : Path('') Args(1) {
  ...
  foreach my $title (@$titles) {
    ...
    $loc->appendChild(
      $doc->createTextNode(
        $c->uri_for_action( 'view/index', $title->{key} )
      )
    );
    $url->appendChild($loc);
...
sub static : Path('static.xml') Args(0) {
  ...
  my $pages = $c->stash->{portal}->pages;
  foreach my $path ( sort keys %$pages ) {
    ...
    foreach my $alternate (@alternates) {
...
sub sitemapindex : Path('sitemap.xml') Args(0) {
  ...
  $loc = $doc->createElement('loc');
  $loc->appendChild(
    $doc->createTextNode( $c->uri_for_action('sitemap/static') ) );
  $map->appendChild($loc);
  ...
  for ( my $i = 0 ; $title_count > 0 ; $title_count -= $map_length ) {
    ...
    $loc->appendChild(
      $doc->createTextNode(
        $c->uri_for_action( 'sitemap/index', [sprintf( "%d.xml", ++$i )] )
      )
    );
    $map->appendChild($loc);
    $root->appendChild($map);
