...
sub index : PathPart('iiif') Chained('/') CaptureArgs(1) {
  ...
  my $doc;
  eval {
    $doc = $c->model('Access::Presentation')->fetch( $slug, $c->portal_id, $c->req->uri->host );
  };
  $c->detach( '/error',
...
sub annotation_page : PathPart('page') Chained('index') Args(2) {
  ...
  my $annotation_page = $c->stash->{doc}->iiif_annotation_page(substr($canvas_name, 1), 1);
...
sub annotation : Chained('index') Args(2) {
  ...
  my $annotation = $c->stash->{doc}->iiif_annotation(substr($canvas_name, 1), 1);
