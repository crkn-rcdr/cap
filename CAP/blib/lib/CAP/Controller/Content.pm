...
{
    my($self, $c) = @_;
    $c->stash->{template} = "content/commit.tt";
...
sub cleanup : Chained('/base') PathPart('content/cleanup') Args(1)
{
    ...
    $c->stash->{template} = "content/cleanup.tt";
    my $dir = join('/', $c->config->{root}, $c->config->{uploads}, $contributor);
    # FIXME: if the directory doesn't exist, this will fail, but it should
    # actually succeed.
    if (! rmtree($dir)) {
...
sub upload : Chained('/base') PathPart('content/upload') Args(0)
{
    ...
    if ($format eq 'application/x-gzip') {
        my @log = ();

        # Find the BagIt root directory (everything up to the first "." in the file name)
        my($root) = ($filename =~ /([^\.]+)/);
        $c->detach('/error', [500, "cannot determine BagIt root directory from file name '$filename'"]) unless ($root);
        ...
        foreach my $file ($bagit->list()) {
            ...
            push(@{$c->stash->{bagit_files}}, $file) if ($file =~ m#^\Q$root\E/data/(metadata|objects)/#);
        }

        $c->stash->{contributor} = $contributor;
        $c->stash->{template} = "content/upload.tt";
        return 1;
    }
...
sub ingest_metadata : Private
{
    ...
    $c->stash->{template} = 'content/ingest_meta.tt';
    return 1;
}
...
sub ingest_object : Private
{
    ...
    $c->detach('/error', [500, "filename '$key' missing required prefix '$prefix'"]) unless
    ...
    $c->stash->{key} = $key;
    $c->stash->{template} = 'content/ingest_object.tt';
    return 1;
}
