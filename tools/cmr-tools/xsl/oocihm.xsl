...
<!--
  ...
  Canadiana.org (oocihm)

  ECO2 metadata 

  Needs a lot of work to bring it up to current standards
...
>
  ...
  <xsl:variable name="default_lang">
    <xsl:call-template name="cmr:iso693_code">
  ...
  <xsl:template match="digital">
    <xsl:variable name="type">document</xsl:variable>
    <record>
      ...
      <xsl:if test="/eco2/@parent">
        <seq><xsl:value-of select="substring-after(substring-after(/eco2/@id, '_'), '_')"/></seq>
      </xsl:if>
  ...
  <xsl:template name="main_description">
    ...
    <xsl:for-each select="//marc/field[@type='600' or @type='610' or @type='630' or @type='650' or @type='651']">
      <xsl:variable name="lang">
        <xsl:choose>
          <xsl:when test="@i2 = '6'">fra</xsl:when>
